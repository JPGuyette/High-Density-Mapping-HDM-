function varargout=hdm2_standalone(dataset,framerate,frames,subsize,subshift,varargin)
% Compiled by HORUS\John T Favreau on 11/27/2013 19:48:43
% hdm2 Version 2013.04.0005
% 
...
%% Process inputs
    ...
    p.addParamValue('roitype','impoly',...
          @(x) any(strcmp(x,{'impoly','imrect','imfreehand'})));
...
%% Process image sequence in order
    ...
    tarind=ismember(fileloc.number,frames);
        ...
        switch comp
            case 'seq'
                targetroi=targetroi+repmat(temphdm.xxyyshift([1 3]),...
                    [size(targetroi,1),1]); %Don't shift ROI
        end
...
function sse=esinc_fit(params,c)
 ...
 function [varargout]=getsubimages(pos,im1,subsize,subshift,roitype)
        ...
        if strcmp(roitype,'imrect')
            pos1=[pos(1,1)-1,pos(1,3)-1;pos(1,2)+1,pos(1,3)-1;...
                pos(1,2)+1,pos(1,4)+1;pos(1,1)-1,pos(1,4)+1;];
            pos=pos1;
        end
...
if exist([tardir,'srchash_',ext,'.mat'],'file');
    ...
    function calchash
        ...
        bar=waitbar(0,'Calculating image hash values...');
        tstart=now();
        for i=1:N
            ...
            waitbar(i/N,bar,['Image hash calc time remaining: '...
...
%       Current options are: 'foroosh'(default), 'esinc', 'hdm', and 'none'
    ...
    if isa(subim1,'integer')
        subim1=double(subim1)/255;
    end
    if isa(subim2,'integer')
        subim2=double(subim2)/255;
    end
...
%     subim2=(subim1-mean(mean(subim2)))/max(max(abs(subim2)));
    ...
    xcorr=xcorr./ab;
    %xcorr(1,1)=0;%??????
    ifft_corr=ifft2(xcorr);
    icorr=fftshift(ifft_corr);
    icorr=real(icorr);
    
    % Find the maximum peak
    [maxy, maxpos]=max(icorr);
    [Cmax, x0]=max(maxy);
    y0=maxpos(x0);
...
%% Populate subimages in the ROI
    ...
    subim=getsubimages(roi,im1,subsize,subshift,'impoly');
...
%% HELPER FUNCTIONS (EMBEDDED)
    ...
    roitype=roitypeL;
        ...
        addNewPositionCallback(h,@disptrueroi);
        roi=getPosition(h);
        if strcmp(roitype,'imrect')
        ...
        function disptrueroi(pos)
            ...
            [U,C,BB,shape]=getsubimages(pos,im1,subsize,subshift,'impoly');
