...
% avg_strain_green Calculates strains from compiled HDM data
...
%   xl: 1 or 0 specifying whether or not to output to excel
%
%   2013 John Favreau
...
% This file is now an alias for avg_strain_green2

if nargout==1
    varargout{1}=avg_strain_green2(dataset,xl);
else
    avg_strain_green2(dataset,xl);
end
...
%     if ischar(dataset)
%         top_dir=[dataset,'\'];
%         if exist([top_dir,'matlab_data\rawhdm.mat'],'file')
%             load([top_dir,'matlab_data\rawhdm.mat']);
%         elseif exist([top_dir,'matlab_data\rawhdm2.mat'],'file')
%             %avg_strain_green2(dataset,xl)
%             return
%         else
...
%     incgrad.amap=zeros(y-1,x-1,time);
%     incstrain.cauchy = zeros(2,2,time);
%     incstrain.green = zeros(2,2,time);
%     strain.cauchy = zeros(2,2,time);
%     strain.green = zeros(2,2,time);
%     [X,Y]=meshgrid(0:rawhdm.shift:x*rawhdm.shift-rawhdm.shift,...
%         0:-rawhdm.shift:-y*rawhdm.shift+rawhdm.shift);%---------------------------Strain issue!
...
%         U=rawhdm.udata(:,:,f);
%         V=rawhdm.vdata(:,:,f);
%         UX=(U-repmat(mean(U,2),[1,x]));
%         UY=(U-repmat(mean(U,1),[y,1]));
%         VX=(V-repmat(mean(V,2),[1,x]));
%         VY=(V-repmat(mean(V,1),[y,1]));
%         % Calculate displacement gradients
%         dudx=mean(sum(UX.*XX)/sum(XX.^2));
%         dudy=mean(sum(UY.*YY)/sum(YY.^2));
%         dvdx=mean(sum(VX.*XX)/sum(XX.^2));
%         dvdy=mean(sum(VY.*YY)/sum(YY.^2));
%         % Store the incremental deformation gradient
%         incgrad.defgrad(:,:,f)=[dudx,dudy;dvdx,dvdy;]+eye(2);
%         
%         % Calculate area strains
%         s=double(rawhdm.shift(1));
...
%                     + U1.*V3 - U3.*(V1+s));
%         incgrad.astretch(f,1)=((mean(adata(:))-s^2)/s^2)+1;
...
%     tfinal=toc;
%     % LOG TO GCE
% %         if exist('GCE_sessionend','file')==2
% %             GCE_logme(['avg_strain_green ',dataset,...
% %                 '-time=',datestr(datenum(0,0,0,0,0,tfinal),'HH:MM:SS')])
% %         end
%     if nargout==1
%         out.time=time;
%         out.strain=strain;
