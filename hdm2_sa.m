...
% Compiled by ADMIN\kjhansen on 01/07/2017 13:54:12
% hdm2 Version 2015.04.0002
% 
...
%% Process inputs
    ...
    if verLessThan('matlab','8.4')
        ...
        p.addParameter('roi',0,@(x)(isnumeric(x) && size(x,2)==2))
        p.addParameter('subpix','foroosh',...
            @(x) any(strcmp(x,{'none','esinc','foroosh','favreau1'})));
        p.addParameter('rigidbodymotion',1,@(x) x==0||x==1)
        p.addParameter('roitype','impoly',...
              @(x) any(strcmp(x,{'impoly','imrect','imsquare','imcircle',...
              'imellipse'})));
        p.addParameter('comparison','seq',...
            @(x) any(strcmp(x,{'seq','each'})))
        p.addParameter('showprogress',1,@isnumeric)
        p.addParameter('importrbm',0,@(x) x==0||x==1||x==2)
        p.addParameter('checkframerate',1,@(x) any(x==[0 1]))
    end
...
function sse=esinc_fit(params,c)
 ...
 function [varargout]=getsubimages(pos,im1,subsize,subshift)
        ...
        if numel(im1)==2
            mask=poly2mask(pos(:,1),pos(:,2),im1(1),im1(2));
        else
        ...
        for I =1:length(xarray)
            for J=1:length(yarray)
                ...
                if all(cursub)
                        ...
                        U2(j:j+subsize(1)-1,i:i+subsize(2)-1)=true;
        ...
        switch nargout
            case 4
                varargout={U2,[xc,yc],R.BoundingBox,...
                    [length(yarray),length(xarray)]};
            case 1
                getsubimout.pixX=X;
                getsubimout.pixY=Y;
                getsubimout.use=U;
                getsubimout.realmask=U2;
...
top_dir=[dataset,'\'];
    ...
    elseif exist('tardir','var')
        ...
        if nargin==4
            ...
            tardir=uigetdir('\\research.wpi.edu\gaudettelab\Raw Data\',ith the target files');
            list=dir([tardir,'\*',ext]);
        end
        mkfilelist(dataset,tardir,list,ext);
        if nargin==4
